...
class GSFCDailyFile(DailyFile):
    ...
    def compute_dac(self, unique_cycles: np.ndarray, ssh: np.ndarray) -> np.ndarray:
        ...
        for cycle_num in unique_cycles:
            noib_filename = f'Merged_TOPEX_Jason_OSTM_Jason-3_Sentinel-6_Cycle_{cycle_num}.V5_2.nc'
            src = os.path.join('s3://', 'example-bucket', 'ref_files', 'gsfc_noib', noib_filename)
            try:
                obj = aws_manager.stream_s3(src)
    ...
    def assign_flags(self, nasa_flag, median_flag, source_flag, all_flags, flag_list):
        ...
        for i, src_flag in enumerate(flag_list, 1):
            source_flag_attrs[f'flag_column_{i}_meaning'] = '0 for good data, 1 for flag is True'

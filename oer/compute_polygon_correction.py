...
def create_polygon(xover_ds: xr.Dataset, date: datetime, satellite: str):
    ...
    if len(iilimit) == 0:
        ...
        trackid_pad = np.concatenate((np.full_like(hpmin, trackid[iitoday[0]]),
                                      trackid[iitoday],
    ...
    ds = xr.Dataset(
        ...
        attrs = {
            "title": f"{satellite} Spline coefficents for Orbit Error Reduction",
            "subtitle": f"created for {satellite} {date}"
        }
    )
    return ds
...
def evaluate_correction(polygon_ds: xr.Dataset, daily_file_ds: xr.Dataset, date: datetime, satellite: str): 
    ...
    ds = xr.Dataset(
        ...
        attrs={
            ...
            "subtitle": f'created for {satellite}-alt_ssh{date.strftime("%Y%m%d")}.nc'
        }
    )
...
def apply_correction(daily_file_ds: xr.Dataset, correction_ds: xr.Dataset):
    ...
    if len(correction_ds.time) != len(daily_file_ds.time):
        raise ValueError(f'Unable to apply correction. Differing sizes between correction and daily file.')

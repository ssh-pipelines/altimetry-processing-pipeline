...
OERFIT_RESULT = Tuple[np.ndarray, np.ndarray,
...
# Validate dimensions of input
    ...
    if np.size(dssh, axis=0) != np.size(trackid, axis=0):
        raise ValueError(
            "Input arrays dssh and trackid should be the same size")
    ...
    gaps = np.diff(pt)
    smallgapind = np.where((gaps > .75) & (gaps < 1.2))[0]
    biggapind = np.where(gaps >= 1.2)[0]
    gapcnst = np.append(pt[smallgapind]+.5*gaps[smallgapind],
                        pt[biggapind]-.5)
    gapcnst = np.append(gapcnst, pt[biggapind]+.5)

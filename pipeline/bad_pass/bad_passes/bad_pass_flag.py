...
class XoverProcessor:
    def __init__(self, source: str, start_date: str = None, end_date: str = None):
        ...
        self.fs = s3fs.S3FileSystem(
            anon=False, key=self._access_key, secret=self._secret_key, token=self._session_token
        )
    ...
    def get_files(self) -> Iterable[str]:
        all_files = sorted(self.fs.glob(os.path.join(self.xover_pattern, self.source, "**", "*.nc")))
        if not self.start_date and not self.end_date:
            logging.info(f"Found {len(all_files)} xover files between {self.start_date} and {self.end_date}")
            return all_files
    ...
    def identify_bad_passes(
        ...
        bad_tid_list = []
        ii = np.where((psec >= currentdate - 3600) & (psec <= currentdate + 86400 + 3600))[0]
    ...
    def scan_source(self) -> pd.DataFrame:
        ...
        for i, file in enumerate(self.files):
            ...
            if bad_passes:
                bad_tid_list.extend([f"{self.source},{cdate_str},{cycle},{pass_num}" for cycle, pass_num in bad_passes])
            else:
        ...
        mask = (self.bad_pass_df["date"].isin(dates_to_drop)) & (self.bad_pass_df["source"] == self.source)
        self.bad_pass_df = self.bad_pass_df.drop(index=self.bad_pass_df[mask].index)

        bad_tid_dfs = [pd.DataFrame([row.split(",")], columns=self.bad_pass_df.columns) for row in bad_tid_list]
        self.bad_pass_df = pd.concat([self.bad_pass_df] + bad_tid_dfs).reset_index(drop=True).drop_duplicates()

        return self.bad_pass_df

    def _load_bad_pass_csv(self) -> pd.DataFrame:
        stream = self.fs.open(self.out_file)
...
def update_bad_passes(gsfc_start: str, gsfc_end: str, s6_start: str, s6_end: str):
    ...
    if gsfc_start and gsfc_end:
        logging.info(f"Checking GSFC between {gsfc_start} and {gsfc_end} for bad passes")
        xover_processor = XoverProcessor("GSFC", start_date=gsfc_start, end_date=gsfc_end)
    ...
    if gsfc_bad_pass_df is not None and s6_bad_pass_df is not None:
        combined_bad_pass_df = pd.concat([gsfc_bad_pass_df, s6_bad_pass_df]).drop_duplicates().reset_index(drop=True)
    ...
    xover_processor.fs.upload("/tmp/bad_pass_list.csv", xover_processor.out_file)

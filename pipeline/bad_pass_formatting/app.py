...
def remove_good_passes(df: pd.DataFrame, item: Dict) -> pd.DataFrame:
    logging.info(f"Checking if {item['source']} on {item['date']} was previously flagged...")
    rows_to_drop = df.loc[(df["source"] == item["source"]) & (df["date"] == item["date"])]
    if rows_to_drop.size > 0:
        ...
        logging.info(f"No previously flagged passes for {item['source']} on {item['date']}")
    return df

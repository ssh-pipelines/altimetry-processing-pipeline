...
class OerTestCase(unittest.TestCase):
    ...
    def stream_xovers(
        window_start: datetime, window_end: datetime, satellite: str
    ) -> xr.Dataset:
        logging.info(
            f"Streaming crossover files between {window_start}, {window_end}..."
        )
        files = sorted(glob("tests/test_xovers/*.nc"))
        files = filter(
            lambda x: os.path.basename(x)
            >= f'xovers_{satellite}-{window_start.strftime("%Y-%m-%d")}.nc',
            files,
        )
        files = filter(
            lambda x: os.path.basename(x)
            <= f'xovers_{satellite}-{window_end.strftime("%Y-%m-%d")}.nc',
            files,
        )
    ...
    def setUpClass(cls):
        ...
        window_start = max(
            date - timedelta(window_len) - timedelta(window_pad), datetime(1992, 9, 25)
        )
        ...
        cls.daily_file_ds = xr.open_dataset(
            "tests/test_dailyfiles/GSFC-alt_ssh19921101.nc"
        )
        cls.correction_ds = evaluate_correction(
            cls.polygon_ds, cls.daily_file_ds, date, satellite
        )
        ...
        if "time" in cls.corrected_ds["basin_names_table"].dims:
            if cls.corrected_ds["basin_names_table"].time.size > 0:
                cls.corrected_ds["basin_names_table"] = cls.corrected_ds[
                    "basin_names_table"
                ].isel(time=0)
            else:
                cls.corrected_ds["basin_names_table"] = cls.corrected_ds[

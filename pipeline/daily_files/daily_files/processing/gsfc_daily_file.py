...
class GSFCDailyFile(DailyFile):
    def __init__(
        ...
        dac: np.ndarray = np.full_like(
            self.og_ds.ssha.values, 0
        )  # Place holder until real DAC data is available
    ...
    def compute_dac(self, unique_cycles: np.ndarray, ssh: np.ndarray) -> np.ndarray:
        ...
        for cycle_num in unique_cycles:
            noib_filename = (
                f"Merged_TOPEX_Jason_OSTM_Jason-3_Sentinel-6_Cycle_{cycle_num}.V5_2.nc"
            )
            src = os.path.join(
                "s3://", "example-bucket", "ref_files", "gsfc_noib", noib_filename
            )
            try:
    ...
    def make_nasa_flag(self):
        ...
        self.ds["nasa_flag"] = (
            ...
            nasa_flag.data,
            {
                ...
                f'to 0: {", ".join([all_flags[i] for i in [1,2,3,5]])}'
            },
        )
        ...
        source_flag_attrs = {
            ...
            "comment": "GSFC flags used to calculate nasa_flag",
        }
        for i, src_flag in enumerate(flag_list, 1):
            source_flag_attrs[f"flag_column_{i}"] = src_flag
        for i, src_flag in enumerate(flag_list, 1):
            source_flag_attrs[f"flag_column_{i}_meaning"] = (
                "0 for good data, 1 for flag is True"
            )
        self.ds["source_flag"] = (
            ("time", "src_flag_dim"),
            source_flag,

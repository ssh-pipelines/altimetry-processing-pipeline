...
def ssha_smoothing(ds: xr.Dataset, date: datetime) -> xr.Dataset:
    ...
    df = pd.DataFrame({"ssha": ds["ssha"].values, "flag": ds["nasa_flag"].values}, ds["time"].values)
    ...
    ssha_smoothed = pd.Series(smoothed_vals, index=padded_df.index)[pd.DatetimeIndex(ds["time"].values)]

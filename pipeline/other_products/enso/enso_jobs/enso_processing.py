...
def start_job(date: datetime):
    ...
    key = os.path.join("s3://example-bucket/simple_grids/p3", str(date.year), filename)
    try:
    ...
    try:
        ...
        src = f"/tmp/{filename}"
        dst = f"s3://example-bucket/enso_grids/{filename}"
        ...
        src = f"/tmp/{filename}"
        dst = f"s3://example-bucket/maps/enso_maps/ortho/{filename}"
        aws_manager.upload_obj(src, dst)

        filename = f'ENSO_plate_{date_str}.png'
        src = f"/tmp/{filename}"
        dst = f"s3://example-bucket/maps/enso_maps/plate/{filename}"
        aws_manager.upload_obj(src, dst)
